#!/bin/bash
# hooks/opencode-pre-tool-use.sh
# Fires before EVERY tool call the agent makes in OpenCode.
...
if [[ "$TOOL" =~ ^(read|view|edit|create|write|str_replace|insert|webfetch|websearch|codesearch|external_directory|doom_loop)$ ]] || \
   [[ "$TOOL" == "bash" ]]; then

  TARGET_PATH=$(echo "$ARGS" | jq -r '.path // .file // .command // .url // ""' 2>/dev/null || echo "$ARGS")
