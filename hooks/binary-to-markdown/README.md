...
| Antigravity CLI | BeforeTool shell hook (`antigravity.sh` / `antigravity.ps1`) | Partial | Converts and logs guidance, but Antigravity cannot accept injected replacement content. |
...
- Copy this folder to your hook location for Claude/Antigravity (for example `~/.claude/hooks/binary-to-markdown/`, `~/.gemini/hooks/binary-to-markdown/`).
- Copy OpenCode plugin file to `~/.config/opencode/plugins/binary-to-markdown.js`.

### 2) Make shell scripts executable

```bash
chmod +x claude-code.sh antigravity.sh
```
...
Set `GEMINI_HOOK_COMMAND` (or your equivalent Antigravity hook command setting) to:

```bash
bash ~/.gemini/hooks/binary-to-markdown/antigravity.sh
```

```powershell
pwsh -File "$HOME/.gemini/hooks/binary-to-markdown/antigravity.ps1"
```
...
- Antigravity cannot inject converted content back into the tool call; it can only log guidance.
